...
public class ZeroArrayTransformationII {
    ...
    public int minZeroArray(int[] nums, int[][] queries) {
        ...
        // Check if all queries can satisfy nums
        ...
        while (l <= r) {
            ...
            if (check(nums, queries, middle)) {
                r = middle - 1; // Try fewer queries
            } else {
                l = middle + 1; // Increase the number of queries
            }
        }
        return l; // Minimum number of queries needed
    }

    // Helper function to check if `k` queries can satisfy the array
    private boolean check(int[] nums, int[][] queries, int k) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            diff[start] += val; // Increment at start
            if (end + 1 < n) {
                diff[end + 1] -= val; // Decrement after end
            }
        }
        // Check if the cumulative sums can satisfy nums
        int cumulative = 0;
        for (int i = 0; i < n; i++) {
            cumulative += diff[i]; // Update with cumulative sum
            if (cumulative < nums[i]) { // Condition not satisfied
                return false;
            }
        }
