...
public class InsertInterval {
    ...
    public int[][] insert(int[][] intervals, int[] newInterval) {
        ...
        while (i < n && intervals[i][1] < newInterval[0]) { //i < n && [i][1] < [0]
            res.add(intervals[i]);
            i++;
        }
        // Middle part (overlapping intervals with newInterval)
        while (i < n && intervals[i][0] <= newInterval[1]) { //i < n && [i][0] <= [1]
            newInterval[0] = Math.min(newInterval[0], intervals[i][0]); // [0], [i][0]
            newInterval[1] = Math.max(newInterval[1], intervals[i][1]);//  [1], [i][1]
            i++;
        }
